...
RUN apt-get -y update && apt-get -y upgrade && \
    ...
    cd /usr/local && \
    curl https://nodejs.org/dist/v0.12.5/node-v0.12.5-linux-x64.tar.gz | tar --strip-components=1 -zxf- && cd && \
    npm -g update npm && \
...
RUN npm install
EXPOSE 8080
CMD forever start -a --uid fake-registry app.js 8080 && \
    forever -a --uid fake-registry2 app.js 8081
